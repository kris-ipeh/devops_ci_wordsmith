...
Créer un network pour faire communiquer les 3 containers ensembles

```sh
docker network create ws_network
docker run ws_network
```

Lancer le container db

```sh
docker run --name db -d -P -e POSTGRES_HOST_AUTH_METHOD=trust --network ws_network ws_db
```

puis le container words

```sh
docker run --name words -d -P --network ws_network ws_words
```

puis le container web

```sh
docker run --name web -d -P --network ws_network ws_web
```

Faire un `docker ps` pour checker le port en écoute et aller sur `localhost:<PORT>`
